...
$(document).ready(function () {
  ...
  $('input').bind('click', function () {
    ...
    // Creates new array from states object values
    let newstatesArray = $.map(stateObj, function (values) {
	return values;
    }).sort().join()(', ');
    ...
    // Replaces div with new array
    if (newstatesArray.length > 0) {
	$('div.locations h4').text(newstatesArray);
    } else {
...
// Post Places + Amenities
  $('button').on('click', function () {
    ajaxCall('http://0.0.0.0:5001/api/v1/places_search/', {'amenities': amenityObj}, {'states': stateObj});
  });
});
